...
<c:if test="${empty aemail}">
	...
	<title>QT-Shop | Đơn hàng đã giao </title>
...
<h3 class="text-center">Đơn hàng đã giao</h3>
	<table id="example" class="table table-hover table-bordered text-center">
          ...
          <br>${orders.product.name}</td>
							...
							<td><a href="${pageContext.request.contextPath}/admin/order/changeStatus/${orderStatus}/${orders.id}" class="btn btn-warning" onclick="return confirm('Đổi trạng thái thành đã xử lý?');">Đã xử lý</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
