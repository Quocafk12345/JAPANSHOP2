...
<c:if test="${empty aemail}">
	...
	<title>QT-Shop | Đơn hàng đang xử lý </title>
...
<c:if test="${not empty delivered}">
	<div class="alert alert-success alert-dismissible fade show text-center">
		Tình trạng đơn hàng đã thay đổi thành đang chờ xử lý.
		<button type="button" class="close" data-dismiss="alert">&times;</button>
	</div>	
</c:if>
<p id="mb">Kéo qua phải để xem chi tiết.</p>
<h3 class="text-center">Đơn hàng đang xử lý</h3>
	<table id="example" class="table table-hover table-bordered text-center">
				...
				<c:set var="orderStatus" value="Đã giao" scope="page"></c:set>
          ...
          <br>${orders.product.name}</td>
							...
							<td><a href="${pageContext.request.contextPath}/admin/order/changeStatus/${orderStatus}/${orders.id}" class="btn btn-success" onclick="return confirm('Thay đổi trạng thái đơn hàng thành đã giao hàng?');">Đang giao</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
