	...
	String getPdf(HttpServletRequest request, HttpSession session, RedirectAttributes rda) {
		try {
			...
			if (email != null) {
				...
				p2.add(new Paragraph("Chi tiết đơn hàng", smallBold));
				p2.add(new Paragraph("Mã đơn hàng: " + orderNumber, smallBold));
				p2.add(new Paragraph("Số tiền: VND. " + totalAmount, smallBold));
				p2.add(new Paragraph("Ngày đặt hàng: " + orderDate, smallBold));
				p2.setAlignment(Element.ALIGN_RIGHT);
				addEmptyLine(p2, 1);
				document.add(p2);
				Paragraph p3 = new Paragraph();
				p3.add(new Paragraph("Chi tiết khách hàng", smallBold));
				p3.add(new Paragraph("Tên khách hàng: " + name, smallBold));
				p3.add(new Paragraph("Địa chỉ: " + address, smallBold));
				p3.add(new Paragraph("Mã vùng: " + pinCode, smallBold));
				p3.add(new Paragraph("Số điện thoại: " + phone, smallBold));
				p3.setAlignment(Element.ALIGN_LEFT);
				addEmptyLine(p3, 2);
				document.add(p3);
				String[] headers = new String[] { "STT", "Tên sản phẩm", "Số lượng.", "Giá đã giảm(VNĐ).", "Giá bán.",
						"Tổng tiền (VNĐ.)", "Số tiền đã giảm (VNĐ.)" };
				...
				addEmptyLine(p9, 1);
				p9.add(new Paragraph("Điều kiện", smallBold));
				p9.add(new Paragraph(
						"1. Giá và số lượng mặt hàng có thể thay đổi và chúng tôi sẽ lập hóa đơn theo giá hiện có trong ngày.",
						smallBold));
				p9.add(new Paragraph("2. Vui lòng nhận hóa đơn của bạn trong khi chờ đơn hàng của bạn.", smallBold));
				p9.add(new Paragraph(
						"3. Đây không phải là hóa đơn Thực hiện hoặc hóa đơn. Đây chỉ là báo cáo về đơn hàng đã nhận.",
						smallBold));
				document.add(p9);
				document.close();
